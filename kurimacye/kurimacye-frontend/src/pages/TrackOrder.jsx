    ...
    <div className="min-h-screen bg-cream-100 dark:bg-charcoal-900 transition-colors duration-300">
      ...
      <main>
        ...
        <div className="mx-auto max-w-3xl px-4 py-10">
          <div className="bg-white dark:bg-charcoal-800 rounded-2xl shadow-sm border border-cream-200 dark:border-charcoal-700 p-5 md:p-7 mb-6">
            <form onSubmit={submit} className="space-y-4">
              <div className="flex flex-col sm:flex-row gap-3">
                <div className="flex-1 relative">
                  <FaSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-charcoal-400 text-sm" />
                  <input
                    type="text"
                    value={query}
            ...
            {error && (
              <div className="mt-4 p-4 bg-terracotta-50 dark:bg-terracotta-900/10 border border-terracotta-500/20 rounded-xl flex items-center gap-3 text-sm text-terracotta-600 dark:text-terracotta-400 font-bold">
                <FaClipboardList />
                {error}
              </div>
            )}
          ...
          {result && (
            <div className="bg-white dark:bg-charcoal-800 rounded-2xl shadow-xl border border-cream-200 dark:border-charcoal-700 overflow-hidden animate-fade-in-up">
              ...
              <div className="p-5 md:p-7 bg-cream-50/50 dark:bg-charcoal-700/50">
                    ...
                    <div className="space-y-4 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-terracotta-500/20 before:to-transparent">
                      ...
                      {[
                        ...
                        return (
                          <div key={stage.status} className={`relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group ${isCompleted ? 'is-active' : 'opacity-50'}`}>
                            <div className={`flex items-center justify-center w-8 h-8 rounded-full border border-white shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 ${isCompleted ? 'bg-terracotta-500 text-white' : 'bg-cream-300 text-charcoal-400'}`}>
                              {isCompleted ? <FaCheckCircle className="text-sm" /> : <div className="w-1.5 h-1.5 rounded-full bg-charcoal-400"></div>}
                            </div>
                            <div className="w-[calc(100%-3rem)] md:w-[calc(50%-2rem)] p-3.5 rounded-xl bg-white dark:bg-charcoal-800 border border-cream-200 dark:border-charcoal-700 shadow-sm transition-all hover:shadow-md">
                              <div className="flex items-center justify-between space-x-2 mb-0.5">
                                <div className={`text-sm font-black ${isCompleted ? 'text-charcoal-800 dark:text-white' : 'text-charcoal-400'}`}>{stage.label}</div>
                                {stage.status === 'pending' && (
                                  <time className="font-mono text-[10px] font-bold text-terracotta-500 dark:text-terracotta-400">{new Date(result.createdAt).toLocaleDateString()}</time>
                                )}
                              </div>
                              <p className="text-xs text-charcoal-500 dark:text-charcoal-400 font-medium">{stage.desc}</p>
                            </div>
                          </div>
                        );
                      })}
                      ...
                      {result.notes?.filter(n => n.isCustomerVisible).map((note, idx) => (
                        <div key={idx} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
                          <div className="flex items-center justify-center w-8 h-8 rounded-full border border-white bg-sand-500 text-white shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2">
